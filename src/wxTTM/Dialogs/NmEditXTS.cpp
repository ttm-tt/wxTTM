...
IMPLEMENT_DYNAMIC_CLASS(CNmEditXTS, CFormViewEx)

BEGIN_EVENT_TABLE(CNmEditXTS, CFormViewEx)
  EVT_LIST_ITEM_SELECTED(XRCID("NominatedPlayers"), CNmEditXTS::OnSelChanged)
END_EVENT_TABLE()


// LtItem with sex
class LtItemXTS : public LtItem
{
  public:
    LtItemXTS(bool showNaName = true) : LtItem(showNaName) {};
    LtItemXTS(const LtEntry &lt, bool showNaName = true) : LtItem(lt, showNaName) {};

  public:
    virtual void DrawItem(wxDC *pDC, wxRect &rect) override;
};

void LtItemXTS::DrawItem(wxDC* pDC, wxRect& rect)
{
...
CNmEditXTS::CNmEditXTS() : CFormViewEx()
{
}


CNmEditXTS::~CNmEditXTS()
{
}


bool  CNmEditXTS::Edit(va_list vaList)
{
...
// -----------------------------------------------------------------------
void CNmEditXTS::OnInitialUpdate() 
{
...
// -----------------------------------------------------------------------
void  CNmEditXTS::OnSelChanged(wxListEvent &)
{
  ...
  // 2nd and 4th match is a girl, also the 2nd player in the double
  bool wantGirl = nmList->GetCurrentIndex() == 1 || nmList->GetCurrentIndex() == 3 || nmList->GetCurrentIndex() == 0 && nmItemPtr->nm.ltA != 0;
  // 3rd and 5th match is a boy, also the 1st player in the double
  bool wantBoy  = nmList->GetCurrentIndex() == 2 || nmList->GetCurrentIndex() == 4 || nmList->GetCurrentIndex() == 0 && nmItemPtr->nm.ltA == 0;
  ...
  while (lt.Next())
  {
    ...
    LtItem *itemPtr = new LtItemXTS(lt, false);
...
void  CNmEditXTS::OnAdd()
{
  ...
  if (nmItemPtr->AddPlayer(LtEntry(lt, pl)))
  {
    ...
    // If double then also insert into singles
    if (nmItemPtr->nm.team.cpType == CP_DOUBLE)
    {
      if (pl.psSex == SEX_FEMALE)
        ((NmItem *) nmList->GetListItem(3))->AddPlayer(LtEntry(lt, pl));
      else
        ((NmItem *) nmList->GetListItem(4))->AddPlayer(LtEntry(lt, pl));
    }
    else
    {
      // Singles in 4th and 5th match will also play the doubles
      if (nmList->GetCurrentIndex() == 3 || nmList->GetCurrentIndex() == 4)
        ((NmItem *) nmList->GetListItem(0))->AddPlayer(LtEntry(lt, pl));
    }
  }
...
void  CNmEditXTS::OnDelete()
{
  ...
  LtItem *ltItemPtr = new LtItemXTS(lt, false);
