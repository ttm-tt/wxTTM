...
// Spieler (in KO-Raster) fortschreiben
    tmp->ExecuteUpdate(str = 
        "CREATE OR ALTER PROCEDURE mtUpdateRasterProc \n"
        "   @mtNr AS " + INTEGER + ", \n"
        ...
        "   SET @mtChecked = (SELECT IIF(mtCheckMatchTime IS NULL, 0, 1) FROM MtRec WHERE mtNr = @mtNr); \n"
...
// Ergebnis in die DB eintragen
    tmp->ExecuteUpdate(str = 
        "CREATE OR ALTER PROCEDURE mtSetResultProc \n"
        "       @mtNr " + INTEGER + ", \n"
        "       @mtMS " + SMALLINT + ", \n"
        "       @mtBestOf " + SMALLINT + ", \n"
        "       @mtSets " + VARCHAR + "(40), \n"
        ...
        "   DECLARE @cpType " + INTEGER + ";\n"
        "   DECLARE @grModus " + SMALLINT + ";\n"
        "   DECLARE @syComplete " + SMALLINT + ";\n"
        "   DECLARE @minRes " + SMALLINT + ";\n"
        ...
        "   SET @cpType = (SELECT cpType FROM CpRec WHERE cpID = @grID); \n"
        "   SET @grModus = (SELECT grModus FROM GrRec WHERE grID = @grID); \n"
        ...
        "   IF ((@cpType = 4) AND (@grModus = 1)) \n"
        "   BEGIN \n"
        "     SET @syID = (SELECT syID FROM GrRec WHERE grID = @grID); \n"
        "     SET @syComplete = (SELECT syComplete FROM SyRec WHERE syID = @syID); \n"
        "   END \n"

        "   IF (@syComplete > 0) \n"
        "     SET @minRes = (SELECT syMatches FROM SyRec WHERE syID = @syID); \n"
        "   ELSE IF (@mtMatches > 1) \n"
        "     SET @minRes = FLOOR(@mtMatches / 2); \n"
        "   ELSE \n"
        "     SET @minRes = FLOOR(@mtBestOf / 2); \n"
        ...
        "    WHERE mtID = @mtID AND \n"
        "          mtEndMatchTime IS NOT NULL AND \n"
        ...
        "           (mtMatches > 1 AND 2 * mtResA < mtMatches AND 2 * mtResX < mtMatches)) AND \n"
        "          (mtResA + mtResX < @minRes) \n"
        
        "   UPDATE MtRec SET mtEndMatchTime = GETUTCDATE() \n"
        "    WHERE mtID = @mtID AND \n"
        "          mtEndMatchTime IS NULL AND \n"
        "          (mtWalkOverA > 0 OR mtWalkOverX > 0 OR \n"
        "           mtInjuredA > 0 OR mtInjuredX > 0 OR \n"
        "           mtDisqualifiedA > 0 OR mtDisqualifiedX > 0 OR \n"
        "           ((mtMatches = 1 AND (2 * mtResA >= mtBestOf OR 2 * mtResX >= mtBestOf)) OR \n"
        "            (mtMatches > 1 AND (2 * mtResA >= mtMatches OR 2 * mtResX >= mtMatches)))) AND \n"
        "            (mtResA + mtResX >= @minRes) \n"
        ...
        "   BEGIN \n"
        "     IF ( (SELECT COUNT(*) FROM MtRec mt WHERE grID = @grID AND mtEndMatchTime IS NULL) = 0)\n"
        "     BEGIN \n"
