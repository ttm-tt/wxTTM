...
// Spieler (in KO-Raster) fortschreiben
    tmp->ExecuteUpdate(str = 
        ...
        "       SET @id = @stA; \n"
        "   END \n"
        "   ELSE IF @stA IS NULL OR @stX IS NULL \n"
        "   BEGIN \n"
        "       SET @id = NULL; \n"
        "   END \n"
        "   ELSE IF @tmX IS NULL \n"
...
// Ergebnis in die DB eintragen
    tmp->ExecuteUpdate(str = 
        ...
        "       BREAK \n"
        "     ELSE IF @mtResA >= @mtResX + 2 AND @mtResA >= 11 \n"
        "       SET @tmp = @tmp + 1;\n"
        "     ELSE IF @mtResX >= @mtResA + 2 AND @mtResX >= 11 \n"
        "       SET @tmp = @tmp + 1;\n"
        ...
        "                     (mtResA >= mtResX + 2 AND mtResA >= 11 OR \n"
        "                      mtResX >= mtResA AND mtResX >= 11); \n" 
        ...
        "                                      mtResA >= mtResX + 2 AND mtResA >= 11), 0), \n"
        "                       ISNULL((SELECT COUNT(*) FROM MtSet \n"
        "                                WHERE mtID = @mtID AND mtMS = @mtMS AND mtSet > 0 AND \n"
        "                                      mtResX >= mtResA + 2 AND mtResX >= 11), 0), \n"
        ...
        "                         ISNULL((SELECT COUNT(*) FROM MtSet \n"
        "                                  WHERE mtID = @mtID AND mtSet > 0 AND mtMS > 0 AND \n"
        "                                        mtResA > mtResX AND mtResA > (@mtBestOf / 2)), 0), \n"
        "                         ISNULL((SELECT COUNT(*) FROM MtSet \n"
        "                                  WHERE mtID = @mtID AND mtSet > 0 AND mtMS > 0 AND \n"
        "                                        mtResX > mtResA AND mtResX > (@mtBestOf / 2)), 0); \n"
        "   END \n"
