...
bool PsRec::Read(const wxString &line)
{
  wxStringTokenizerEx tokens(line, ",;\t");
  wxString strLast  = tokens.GetNextToken();
  wxString strFirst = tokens.GetNextToken();
  wxString strYear  = tokens.GetNextToken();
  wxString strSex   = tokens.GetNextToken();
  wxString strArrvd = tokens.GetNextToken();
  wxString strPhone = tokens.GetNextToken();

  if (!strLast.IsEmpty() && !strFirst.IsEmpty() && !strSex.IsEmpty())
  {
    memset(this, 0, sizeof(PsRec));
    ...
    switch (*strSex.t_str())
    {
      ...
      default :
        psSex = _strtos(strSex);
        break;
    }

    if (!strArrvd.IsEmpty())
    {
      switch (*strArrvd.t_str())
      {
        case 'y' :
        case 'Y' :
        case 'j' :
    ...
    wxStrncpy((wxChar *) psPhone, strPhone, sizeof(psPhone) / sizeof(wxChar) -1);
  }
...
bool PsRec::Write(wxString &str) const
{
  static char sexes[] = {' ', 'm', 'w'};
  static char arrvd[] = {'n', 'y'};
...
bool  PsStore::Import(const wxString &name)
{
  wxTextFile ifs(name);
  if (!ifs.Open())
    return false;
  ...
  // HACK: Variablen duerfen nicht laenger leben als connPtr [
  {
  PsStore  ps(connPtr);
...
bool  PsStore::Export(const wxString &name)
{
  Connection *connPtr = TTDbse::instance()->GetDefaultConnection();
