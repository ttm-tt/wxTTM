...
bool  MtEntryStore::CreateView()
{
  ...
  try
  {
    str = "CREATE VIEW MtSingleList                                             "
          ...
          "          mt.mtPrintTossTime, IIF(mt.mtPrintTossTime IS NULL, 0, 1) AS mtToss,      "
          "          mt.mtPrintScoreTime, IIF(mt.mtPrintScoreTime IS NULL, 0, 1) AS mtPrinted, "
          "          mt.mtStartMatchTime, IIF(mt.mtStartMatchTime IS NULL, 0, 1) AS mtStarted, "
          "          mt.mtEndMatchTime, IIF(mt.mtEndMatchTime IS NULL, 0, 1) AS mtEnded,       "
          "          mt.mtCheckMatchTime, IIF(mt.mtCheckMatchTime IS NULL, 0, 1) AS mtChecked, "
          "          mt.mtTimestamp                                             "
          "     FROM MtRec mt                                                   "
    ...
    str = "CREATE VIEW MtIndividualList                                         "
          ...
          "  plYplID, plYplNr, plYpsLast, plYpsFirst, plYnaID, plYnaName, plYnaDesc, plYnaRegion, plYplExtID,  "
          "  stX, tmXtmID, nmXnmNr,                                             "
          "  mtMatches, mtBestOf, mttmResA, mttmResX, mtUmpire, mtUmpire2,      "
          "  mtPrintTossTime, mtToss,                                           "
          "  mtPrintScoreTime, mtPrinted,                                       "
          "  mtStartMatchTime, mtStarted,                                       "
          "  mtEndMatchTime, mtEnded,                                           "
      ...
      "AS SELECT mt.mtID, mt.mtNr,                                              "
          ...
          "          mt.mtMatches, mt.mtBestOf, mt.mtResA, mt.mtResX, mt.mtUmpire, mt.mtUmpire2, "
          "          mt.mtPrintTossTime, IIF(mt.mtPrintTossTime IS NULL, 0, 1) AS mtToss,      "
          "          mt.mtPrintScoreTime, IIF(mt.mtPrintScoreTime IS NULL, 0, 1) AS mtPrinted, "
          "          mt.mtStartMatchTime, IIF(mt.mtStartMatchTime IS NULL, 0, 1) AS mtStarted, "
          "          mt.mtEndMatchTime, IIF(mt.mtEndMatchTime IS NULL, 0, 1) AS mtEnded,       "
...
bool  MtEntryStore::RemoveView()
{
  ...
  try
  {
    tmp->ExecuteUpdate("DROP VIEW IF EXISTS MtSingleList");
    tmp->ExecuteUpdate("DROP VIEW IF EXISTS MtDoubleList");
    tmp->ExecuteUpdate("DROP VIEW IF EXISTS MtTeamList");
    tmp->ExecuteUpdate("DROP VIEW IF EXISTS MtIndividualList");
  }
  catch(SQLException &)
  {
  }
  ...
  switch (mt.cpType)
  {
    case CP_SINGLE :
      str = "SELECT cp.cpName, cp.cpType, "
            ...
            "       mtDateTime, mtTable, "
            "       mtMatches, mtBestOf, mtUmpire, mtUmpire2, "
            "       IIF(mtPrintScoreTime IS NULL, 0, 1), IIF(mtCheckMatchTime IS NULL, 0, 1), "
