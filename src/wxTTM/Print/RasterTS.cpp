...
RasterToss::RasterToss(Printer *prt, Connection *ptr)
				 : RasterBase(prt, ptr)
{
};


RasterToss::~RasterToss()
{
};
...
int RasterToss::PrintHeader(const MtEntry &mt, const CRect &rect)
{
  CRect evt = rect;
  CRect date = rect;
  CRect time = rect;
...
int RasterToss::PrintSystem(const SyListRec & sy, const CRect & rect)
{
  // Print match order
...
int RasterToss::PrintNomination(const SyListRec& sy, const CRect& rect, bool ax)
{
